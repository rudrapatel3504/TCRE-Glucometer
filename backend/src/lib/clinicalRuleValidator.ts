...
export function validateLatents(
  ...
  stateKeys.forEach(key => {
    ...
    // Verify Reasoning Engine Final Score == Explainability Final Score
...
export function validateComposites(
  ...
  const isUnstableEligible = cbScore > 50 && hvScore > 50 && (viTrend === 'flat' || viRaw <= 40); 
...
export function validateExplainability(
  ...
  const { explainability, metrics, states, risk } = analysis;
  ...
  const volMatch = summaryText.match(/volatility\s*=\s*(\d+)%?/i) || summaryText.match(/\+(\d+)%/i); 
  if (volMatch) {
  ...
  nodes.forEach(n => {
    n.node.errors.forEach(err => {
      ...
      if (n.name === 'Metric Validation') {
        ...
        fix = 'Verify telemetry preprocessors and math utility wrappers.';
